...
class MinerUWorkerAPI(ls.LitAPI):
    ...
    def setup(self, device):
        ...
        if self.enable_worker_loop:
            ...
            # 心跳线程：处理中任务定期续约 Redis processing set，防止长任务被超时恢复误重入队
            self.heartbeat_thread = threading.Thread(target=self._heartbeat_loop, daemon=True)
            self.heartbeat_thread.start()

    def _heartbeat_loop(self):
        """周期性为当前处理中的任务续约心跳（仅 Redis 启用时生效）。"""
        interval = int(os.getenv("WORKER_HEARTBEAT_INTERVAL", "60"))
        while self.running:
            try:
                task_id = self.current_task_id
                if task_id:
                    self.task_db.heartbeat(task_id, self.worker_id)
            except Exception as e:
    ...
    def _process_task(self, task: dict):
        ...
        try:
            # 0. 取消检查：claim 后、重活开始前若已被取消，直接跳过，不再产出
            if self.task_db.is_cancelled(task_id):
                logger.info(f"🚫 Task {task_id} cancelled before processing, skipping")
                return
            ...
            # 3. PDF 拆分（父任务不再重复拆分：被 reset 回 pending 的 is_parent 任务若再次拆分会产生重复子任务）
            if file_ext == ".pdf" and not parent_task_id and not task.get("is_parent"):
            ...
            # 取消检查：解析耗时较长，完成后若发现已被取消，则丢弃产物、不覆盖 cancelled 状态
            if self.task_db.is_cancelled(task_id):
                logger.info(f"🚫 Task {task_id} cancelled during processing, discarding result")
                try:
                    rp = result.get("result_path")
                    if rp and Path(rp).exists():
                        shutil.rmtree(rp, ignore_errors=True)
                except Exception as e:
                    logger.debug(f"cleanup cancelled result failed: {e}")
                return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--workers-per-device", type=int, default=None)
    ...
    # 并发控制：优先使用 CLI --workers-per-device，未显式传入时回落到 MAX_CONCURRENT_TASKS（默认 1）
    if args.workers_per_device is not None:
        workers_per_device = args.workers_per_device
        source = "cli --workers-per-device"
    else:
        workers_per_device = int(os.getenv("MAX_CONCURRENT_TASKS", "1"))
        source = "env MAX_CONCURRENT_TASKS"

    logger.info(f"⚙️  Concurrency Config: workers_per_device={workers_per_device} (from {source})")
