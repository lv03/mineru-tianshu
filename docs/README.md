...
| [02 · 文件解析方法](./02-文件解析方法.md) | 各引擎与文件类型对应关系、MinerU/PaddleOCR-VL/SenseVoice/视频/去水印/格式引擎的解析方法与选项、`auto` 模式兜底链、输出归一化、**「源文档预览 / 双向定位」格式能力矩阵** |
...
| [04 · 本地开发环境与解析问题修复](./04-本地开发环境与解析问题修复.md) | 原生开发使用 `backend/model` 本地模型、MinerU pipeline 路径拼接修正、双向定位空框（normalizer 选 JSON）、PDF 预览路径、双向定位 bbox [0,1000] 归一化坐标偏移、公式/表格/列表取文与页码过滤、**content_list_v2 块结构与字段对照** |
